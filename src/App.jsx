...
export default function AIIMSSchoolPortalPreview() {
  ...
  const Home = () => {
    ...
    return <main className="overflow-x-hidden">
      ...
      <footer className="bg-blue-700 text-white py-8 sm:py-10 px-4 text-center"><img src={SCHOOL_LOGO} alt="School Logo" className="w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 rounded-full object-cover border-2 border-white" /><h4 className="text-xl sm:text-2xl font-bold mb-3">{SCHOOL_NAME}</h4><p className="opacity-90 text-sm sm:text-base leading-relaxed">Built with love! Copyright @2026 Developed by Daris Nisar</p></footer>
    </main>;
  };

  const Stude<Input value={studentRoll} onChange={(e) => setStudentRoll(cleanRoll(e.target.value))} placeholder="Enter 5 Digit Roll Number" /><button onClick={loginStudent} className="w-full bg-blue-600 text-white p-4 rounded-2xl mt-4 font-bold">Login</button>{studentError && <p className="text-red-600 mt-4 text-center">{studentError}</p>}</div></main>;
  ...
  return <div className="min-h-screen bg-gray-100 text-gray-900 overflow-x-hidden"><header className="bg-white shadow sticky top-0 z-50"><div className="max-w-7xl mx-auto px-3 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4"><div className="flex items-center gap-3 w-full sm:w-auto justify-center sm:justify-start"><img src={SCHOOL_LOGO} alt="School Logo" className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover" /><div className="text-center sm:text-left"><h1 className="text-base sm:text-xl font-bold text-blue-700 leading-tight">{SCHOOL_NAME}</h1><p className="text-[10px] sm:text-xs text-gray-500">{SCHOOL_TAGLINE}</p></div></div><nav className="w-full sm:w-auto grid grid-cols-3 sm:flex gap-2 bg-blue-50/70 p-2 rounded-2xl border border-blue-100 shadow-inner"><button onClick={() => setPage('home')} c
